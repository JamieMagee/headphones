...
def libraryScan(dir=None, append=False, ArtistID=None, ArtistName=None,
    ...
    if not append:
        ...
        for track in del_have_tracks:
            ...
            if not os.path.isfile(encoded_track_string):
                ...
                logger.info('File %s removed from Headphones, as it is no longer on disk' % encoded_track_string.decode(
        ...
        for files in f:
            ...
            if any(files.lower().endswith('.' + x.lower()) for x in headphones.MEDIA_FORMATS):
                ...
                if file_count == 0 and r.replace(dir, '') != '':
                    logger.info("[%s] Now scanning subdirectory %s" % (dir.decode(headphones.SYS_ENCODING, 'replace'),
                ...
                try:
                    ...
                    logger.warning("Cannot read media file '%s', skipping. It may be corrupted or not a media file.",
                                   unicode_song_path)
                    continue
                except IOError:
                ...
                if not check_exist_song:
                    ...
                    if check_exist_song['ArtistName'] != f_artist or check_exist_song['AlbumTitle'] != f.album or \
                                    check_exist_song['TrackTitle'] != f.title:
                        # Important track metadata has been modified, need to run matcher again
                        if f_artist and f_artist != check_exist_song['ArtistName']:
                            new_artists.append(f_artist)
                        elif f_artist and f_artist == check_exist_song['ArtistName'] and check_exist_song[
                            'Matched'] != "Ignored":
                            new_artists.append(f_artist)
                        else:
                            continue
    ...
    logger.info(
        "Found " + str(total_number_of_songs) + " new/modified tracks in: '" + dir.decode(headphones.SYS_ENCODING,
                                                                                          'replace') + "'. Matching tracks to the appropriate releases....")
    ...
    if not append or artistScan:
        ...
        for artist in artists_checked:
            ...
            havetracks = (
                len(myDB.select('SELECT TrackTitle from tracks WHERE ArtistName like ? AND Location IS NOT NULL',
                                [artist])) + len(
                    myDB.select('SELECT TrackTitle from have WHERE ArtistName like ? AND Matched = "Failed"', [artist]))
            )
        ...
        havetracks = len(
            myDB.select('SELECT TrackTitle from tracks WHERE ArtistID=? AND Location IS NOT NULL', [ArtistID])) + len(
            myDB.select('SELECT TrackTitle from have WHERE ArtistName like ? AND Matched = "Failed"', [ArtistName]))
